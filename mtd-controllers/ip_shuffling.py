...
class MTDController:
    ...
    def clear_iptables(self):
        """Clear existing NAT and FORWARD rules"""
    ...
    def setup_forwarding(self):
        """Enable IP forwarding and set up basic forwarding rules"""
        # Enable IP forwarding
        subprocess.run(['sudo', 'sysctl', '-w', 'net.ipv4.ip_forward=1'], capture_output=True)
        
        # Allow established connections
        subprocess.run(['sudo', 'iptables', '-A', 'FORWARD', '-m', 'state', '--state', 'ESTABLISHED,RELATED', '-j', 'ACCEPT'])
        
        # Allow forwarding from/to our network
        subprocess.run(['sudo', 'iptables', '-A', 'FORWARD', '-s', '10.10.1.0/24', '-j', 'ACCEPT'])
        subprocess.run(['sudo', 'iptables', '-A', 'FORWARD', '-d', '10.10.1.0/24', '-j', 'ACCEPT'])
        
    def add_virtual_mapping(self, virtual_ip, real_ip):
        """Add iptables rules for virtual -> real mapping"""
        # Incoming: virtual -> real (DNAT)
        subprocess.run([
            'sudo', 'iptables', '-t', 'nat', '-A', 'PREROUTING',
            '-d', virtual_ip, '-j', 'DNAT', '--to-destination', real_ip
        ])
        
        # Force return traffic through controller (MASQUERADE)
        subprocess.run([
            'sudo', 'iptables', '-t', 'nat', '-A', 'POSTROUTING',
            '-d', real_ip, '-j', 'MASQUERADE'
        ])
        
        # Allow forwarding for this specific mapping
        subprocess.run([
    ...
    def rotate_ips(self):
        ...
        ssh_cmd = [
            'ssh', '-o', 'StrictHostKeyChecking=no',
            f'surada@{self.attacker_ip}',
            f'export FEATURE_HUNTER_RUN=/users/surada/mtd-ai-security/features/latest && /users/surada/mtd-ai-security/features/latest/scripts/epoch_marker.sh'
        ]
        subprocess.run(ssh_cmd, capture_output=True)
    ...
    def show_status(self):
        ...
        print("\nCurrent iptables FORWARD rules:")
        result = subprocess.run(['sudo', 'iptables', '-L', 'FORWARD', '-n'], 
                               capture_output=True, text=True)
        print(result.stdout)
