#!/usr/bin/env python3
"""
Generate NDV experiment configurations.
Creates matched strata with different defender types.
"""
...
def generate_ndv_minimal_grid(p=2, B=200, mu=100, seeds=20):
    """
    Generate minimal NDV grid for initial testing.
    ...
    ndv_families = [
        ...
        # Note: Markov and adversarial would need additional attacker modifications
    ]
    ...
    defender_types = [
        {
            'type': 'idham',
            'mask_capacity': int(n_hosts * 0.3),
        },
        {
            'type': 'static_mask',
            'mask_capacity': int(n_hosts * 0.3),
            'initial_mask_strategy': 'random',
        },
        {
            'type': 'random_mtd',
            'mask_capacity': int(n_hosts * 0.3),
            'remask_interval': 50,  # Match ID-HAM update interval
            'remask_mode': 'uniform',
            'initial_mask_strategy': 'random',
        },
    ]
...
def generate_ndv_full_grid(seeds=20):
    """
    Generate full NDV grid across multiple strata.
    """
    configs = []
    n_hosts = 100
    ...
    # Defender types
    defender_types = [
        {'type': 'idham', 'mask_capacity': int(n_hosts * 0.3)},
        {'type': 'static_mask', 'mask_capacity': int(n_hosts * 0.3), 
         'initial_mask_strategy': 'random'},
        {'type': 'random_mtd', 'mask_capacity': int(n_hosts * 0.3),
         'remask_interval': 50, 'remask_mode': 'uniform',
         'initial_mask_strategy': 'random'},
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate NDV experiment configurations')
    ...
    parser.add_argument('--output-dir', type=str, default='configs/ndv',
    ...
    if args.preset == 'minimal':
        print(f"Generating minimal NDV grid: p={args.p}, B={args.B}, μ={args.mu}")
        configs = generate_ndv_minimal_grid(
            p=args.p, B=args.B, mu=args.mu, seeds=args.seeds
        )
        subdir = output_dir / f"p{args.p}_b{args.B}_mu{args.mu}"
    else:
        print("Generating full NDV grid")
    ...
    print(f"\nTo run minimal grid:")
    print(f"  ls {subdir}/*.yaml | parallel -j 8 python run_experiment.py --config {{}}")
