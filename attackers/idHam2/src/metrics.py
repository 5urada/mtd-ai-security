...
def compute_windowed_metrics(all_metrics: List[Dict], window_size: int) -> List[Dict]:
    ...
    for window_start in range(0, n_episodes, window_size):
        ...
        flow_mods_values = [m['flow_mods_count'] for m in window_metrics]
        ...
        windowed_dict = {
            ...
            'flow_mods_mean': float(np.mean(flow_mods_values)),
...
def compute_success_criteria(
    ...
    # Criterion 3: Flow mods not excessive
    if baseline_aggregated is not None:
        flow_mods_ratio = (
            aggregated['mean_flow_mods'] / 
            (baseline_aggregated['mean_flow_mods'] + 1e-10)
        )
        success['flow_mods_ratio'] = float(flow_mods_ratio)
        success['meets_flow_mods_threshold'] = flow_mods_ratio <= 2.0
    else:
        success['flow_mods_ratio'] = None
        success['meets_flow_mods_threshold'] = None
    
    # Criterion 4: Adaptation lag sufficient
    if aggregated['adaptation_lag_median'] is not None:
    ...
    criteria_results = [
        ...
        success['meets_flow_mods_threshold'],
        success['meets_adaptation_lag_threshold']
    ]
